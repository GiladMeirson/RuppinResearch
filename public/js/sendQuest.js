...
$(document).ready(function () {
  ...
  $("#confirmBatchID").on("click", function () {
    ...
    if (batchID) {
      ...
      const questionobjectToServer = GetQuestionByIDArray(
        CurrentQuestionID_DATA
      );
      ...
      $("#loading").show();
      console.log(
        "Questions before* to be inserted to DB:",
        questionobjectToServer
      );
      console.log(
        "Answers before* to be inserted to DB:",
        answersobjectToServer
      );
      // Compress answers object
      const compressedAnswers = pako.gzip(
        JSON.stringify(answersobjectToServer)
      );
...
function RendderToConfirmModal(modelName, questionArray) {
  ...
  $("#QuestAmountTitle").html(
    `You are going to send ${
      questionArray.length
    } questions <br> temperature: ${$("#temperature-slider").val()}`
  );
}
...
function DataTableEvent() {
  ...
  $("#question-table").on("change", ".row-checkbox", function () {
    const allChecked = DATAtable.rows()
      .nodes()
      .toArray()
      .every(function (tr) {
        return $(tr).find(".row-checkbox").prop("checked");
                  ...
                  .map(
                    ...
                    <div class="answer">
                      ...
                      answer.NormalizedScore
                        ...
                        <div class="answer-body">${formatContent(
                          answer.Body
                        )}</div>
                    </div>
                `
...
function renderToPromptSelect() {
  if (PROMPT_LIST.length > 0) {
    ...
    for (let i = 0; i < PROMPT_LIST.length; i++) {
      ...
      $("#PromptList").append(
        `<option value="${prompt.PromptID}">${prompt.PromptName}</option>`
      );
    }
    ...
    const table = $("#question-table").DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render: function (data, type, row) {
            return (
              calcCost(calcTokens(data), row.modelName || "gpt-4o-mini") + "¢"
            );
          },
        },
      ],
...
const changeListEventHandler = (select) => {
  if (select.value == "o1" || select.value == "gpt-5") {
    Swal.fire({
      title: "Warning",
